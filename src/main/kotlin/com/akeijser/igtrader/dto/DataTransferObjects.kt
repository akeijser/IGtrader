package com.akeijser.igtrader.dto
...
data class MultiNodesDTO (
    ...
    fun toMarkets(): List<Market> {

        val listOfMarkets = mutableListOf<Market>()

        nodes?.forEach { listOfMarkets.add(Market(it.id, it.name)) }

        val empty : List<Market> = emptyList()

        if (nodes.isNullOrEmpty()) {
            return empty
        }

        return nodes.map { market ->
            Market(market.id, market.name) }
    }

    fun toEpics(): List<Epic> {
        //todo should be doable in one statement
...
data class MultipleEpicDetailsDTO (
    ...
    fun toDomain(): MultipleEpicDetails{
        val list = mutableListOf<EpicDetail>()
...
data class InstrumentDTO (
    ...
    fun toDomain(): Instrument {
        ...
        return Instrument(
                ...
                , slippageFactor = slippageFactor?.let { SlippageFactor(slippageFactor) }
...
data class OauthTokenDTO (
        ...
        val expires_in : Int
//        , val expiresDateTime : LocalDateTime = LocalDateTime.now().plusSeconds(this.expires_in.toLong() -1)
...
//    private val creationDate =
    ...
    val expiresDateTime: LocalDateTime
        get() {
            println("this is the localDateTime in the getExpiresDateTime ${LocalDateTime.now()}")
            println(expires_in)
            return LocalDateTime.now().plusSeconds(expires_in.toLong() -1)
        }

    init {
        println("this is the localDateTime at init ${LocalDateTime.now()}")
    }
...
data class StreamToken(val cst: String, val xSecurityToken: String, val streamEndpoint: String)