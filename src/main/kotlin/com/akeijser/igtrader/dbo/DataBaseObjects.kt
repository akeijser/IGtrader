package com.akeijser.igtrader.dbo
...
//    val priceDetails : MutableList<PriceDetailsDBO>? = mutableListOf()
    ...
    constructor(domain: Instrument) : this(
            ...
            , slippageFactor = domain.slippageFactor?.let{SlippageFactorDBO(domain.slippageFactor)}
...
//        @JoinColumn(name = "instrument_id", referencedColumnName = "id")
//        @ManyToOne(cascade = [CascadeType.ALL], fetch = FetchType.LAZY)
//        @JoinTable(name = "PriceDetailsDBO_InstrumentDBO", joinColumns ={@JoinColumn(name = "instrument_id", referencedColumnName = "id")} referencedColumnName = "id")
//        val instrument: InstrumentDBO,
        //todo this should reflect epic entity, the table should have a epic_id property/ fix later
        @Column(name = "epic_name")
        val epicName: String,
    ...
    constructor(pricesDetails: PricesDetails): this(
            epicName = pricesDetails.epicName
...
@NamedQueries(
        NamedQuery(name = "oauth_token.find",
                query = "SELECT o FROM OauthTokenDBO o ORDER by creationDateTime desc")
)
data class OauthTokenDBO (
    ...
    constructor(oauthToken: OauthToken): this(
            accessToken = oauthToken.access_token
            , refreshToken = oauthToken.refresh_token
            , scope = oauthToken.scope
            , tokenType = oauthToken.token_type
            , expiresIn = oauthToken.expires_in
            , creationDateTime = oauthToken.creationDateTime
    )
}
